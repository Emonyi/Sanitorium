...
$db = Db::getInstance();
$res = $db->getPDO();
$did=intval(Input::get('id'));// get doctor id
...
$sql = $db->update('doctor',$_GET['id'],array('specialization'=>$docspecialization,
									  'doctorname'=>$docname,
									  'address'=>$docaddress,
									  'docfee'=>$docfees,
...
<?php if($_SESSION['msg']) { echo htmlentities($_SESSION['msg']);}?> </h5>
									...
									<?php 
									$sql=$res->prepare("select * from doctor where id = $did");
									$sql->execute();
									//var_dump($data=$sql->fetch(PDO::FETCH_ASSOC));
